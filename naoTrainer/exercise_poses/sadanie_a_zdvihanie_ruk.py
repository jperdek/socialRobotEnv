...
class SadanieNaStolickuPose(ExerciseBase):
    ...
    def do_check_exercise(self, landmarks, stage_signal, exercise_label_signal, score_signal):
        ...
        if self.sit_stand_raise_arms_sent00 == False:
            stage_signal.emit("00,", "sit_stand_raise_arms_fullfilled", self.camera_exerice_score) # 0. faza - chceme ho dat to vychodzej polohy - signal 00 znaci, aby si sadol
        ...
        if (is_person_standing is False) and (is_person_sitting is True):
            if self.sit_stand_raise_arms_sent01 == False and self.sit_stand_raise_arms_sent00 == True:
                stage_signal.emit("01,", "sit_stand_raise_arms_fullfilled", self.camera_exerice_score)
        ...
        if (is_person_standing is True) and (self.arms_poses.check_arms_raised_up(landmarks) is False):
            if self.sit_stand_raise_arms_sent02 == False and self.sit_stand_raise_arms_sent01 == True and self.sit_stand_raise_arms_sent00 == True:
                stage_signal.emit("02,", "sit_stand_raise_arms_fullfilled", self.camera_exerice_score)
                exercise_label_signal.emit("Clovek sa postavil a ruky ma pri tele.")
                self.sit_stand_raise_arms_sent02 = True

        # Robot zdvihol ruky nad hlavu, cakame, kym to spravi aj clovek
        if (is_person_standing is True) and (self.arms_poses.check_arms_raised_up(landmarks) is True):
            if self.sit_stand_raise_arms_sent03 == False and self.sit_stand_raise_arms_sent02 == True and self.sit_stand_raise_arms_sent01 == True and self.sit_stand_raise_arms_sent00 == True:
                stage_signal.emit("03,", "sit_stand_raise_arms_fullfilled", self.camera_exerice_score)
                exercise_label_signal.emit("V stoji zdvihol ruky nad hlavu.")
                self.sit_stand_raise_arms_sent03 = True

        # Robot ruky vratil, cakame, kym to spravi aj clovek
        if (is_person_standing is True) and (self.arms_poses.check_arms_raised_up(landmarks) is False):
            if self.sit_stand_raise_arms_sent04 == False and self.sit_stand_raise_arms_sent03 == True and self.sit_stand_raise_arms_sent02 == True and self.sit_stand_raise_arms_sent01 == True and self.sit_stand_raise_arms_sent00 == True:
                stage_signal.emit("04,", "sit_stand_raise_arms_fullfilled", self.camera_exerice_score)
                exercise_label_signal.emit("V stoji vratil ruky spat k telu.")
                self.sit_stand_raise_arms_sent04 = True

        if (is_person_standing is False) and (is_person_sitting is True):
            if self.sit_stand_raise_arms_sent05 == False and self.sit_stand_raise_arms_sent04 == True and self.sit_stand_raise_arms_sent03 == True and self.sit_stand_raise_arms_sent02 == True and self.sit_stand_raise_arms_sent01 == True and self.sit_stand_raise_arms_sent00 == True:
                stage_signal.emit("05,", "sit_stand_raise_arms_fullfilled", self.camera_exerice_score)
                # exercise_label_signal.emit("Clovek si sadol naspat na stolicku.")
                self.sit_stand_raise_arms_sent05 = True
