...
class ForefootingOnChair(Exercise):
    def __init__(self, messages, widget, app_instance):
        super(ForefootingOnChair, self).__init__(messages, widget, app_instance)
        self.start_msg = messages['start_msg'] + ","

    def forefooting_on_chair_exercise(self):
        self.start_timer()
        self.camera_thread.frame_captured.connect(self.camera_thread.check_forefooting_on_chair_exercise)
