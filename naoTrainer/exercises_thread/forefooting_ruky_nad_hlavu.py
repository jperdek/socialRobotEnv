...
class ForefootingRukyNadHlavu(Exercise):
    def __init__(self, messages, widget, app_instance):
        super(ForefootingRukyNadHlavu, self).__init__(messages, widget, app_instance)
        self.start_msg = messages['start_msg'] + ","

    def forefooting_ruky_nad_hlavu_exercise(self):
        self.start_timer()
        self.camera_thread.frame_captured.connect(self.camera_thread.check_forefooting_ruky_nad_hlavu_exercise)
