from exercise_poses.tpose_exercise import TposeExercisePose
from .exercise import Exercise
import configuration.exercises as exercise_messages_configuration

class TPoseExercise(Exercise):
    def __init__(self, messages, widget, app_instance):
        super(TPoseExercise, self).__init__(messages, widget, app_instance)
        self.start_msg = messages['start_msg']

    def tpose_exercise(self):
        self.start_timer()

        self.camera_thread.current_exercise = TposeExercisePose()
