...
def should_extract_frame(frame_number: int, fps: int = 30, number_frames_per_sec: int = 1):
    for frame_occurrence in range(0, fps, math.floor(fps / number_frames_per_sec)):
        if frame_number % frame_occurrence == 0:
            return True
    return False
...
def evaluate_yolo_pose(poseweights="yolov7-w6-pose.pt", source: str = "football1.mp4", device='cpu', view_img=False,
    ...
    if not cap.isOpened():  # check if videocapture not opened
        ...
        while cap.isOpened:  # loop until cap opened or video not complete
            ...
            if number_seconds_to_process != -1 and frame_number / fps > number_seconds_to_process:
                print("Number seconds exceeded given threshold of " + str(number_seconds_to_process))
                break
            if ret:  # if success is true, means frame exist
                frame_number = frame_number + 1
                if should_extract_frame(frame_number, fps, number_frames_per_sec):
                    ...
                    for i, pose in enumerate(output_data):  # detections per image
                        ...
                        if len(output_data):  # check if no pose
                            ...
                            for det_index, (*xyxy, conf, cls) in enumerate(
                                ...
                                data_with_poses.append(data_with_pose)
        ...
        print(data_with_poses)
    return data_with_poses
